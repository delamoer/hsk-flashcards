...
async def generate(texts: list[str], manifest: dict) -> int:
    new_count = 0
    for text in texts:
        if text in manifest:
            continue
...
async def main():
    ...
    print(f"Scope: {label} — {len(texts)} unique texts ({len(texts) - sum(t in manifest for t in texts)} new)")
    new_count = await generate(texts, manifest)
