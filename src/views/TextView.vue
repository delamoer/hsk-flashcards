    ...
    <div v-else-if="mode === 'vocab'" class="panel">
      <div class="vocab paper-bg">
        <div class="vhead">
          ...
          <div class="rtoggles" v-if="vHasGloss">
            <button class="rt" :class="{ on: showPy }" @click="showPy = !showPy">拼音<small>Pinyin</small></button>
            <button class="rt" :class="{ on: showEn }" @click="showEn = !showEn">En<small>English</small></button>
          </div>
        </div>
        ...
        <div class="bank">
          ...
          <div class="tiles">
            <button
              ...
              :key="chip.id"
              class="tile"
              :title="vTileEn(chip.text)"
              @click="placeVocab(chip)"
            >
              <span class="tw">{{ chip.text }}</span>
              <span v-if="showPy && vTilePy(chip.text)" class="tpy" v-html="vTilePy(chip.text)"></span>
              <span v-if="showEn && vTileEn(chip.text)" class="ten">{{ vTileEn(chip.text) }}</span>
            </button>
...
// Word-bank tiles pull pinyin + English from the text's baked-in `vocab.tiles`
// (generated by convert_texts.py from the full flashcard vocab + word_gloss.tsv),
// so every 生词 tile can show tone-colored pinyin + its English meaning.
const vTiles = computed(() => text.value?.vocab?.tiles || {});
const vHasGloss = computed(() => Object.keys(vTiles.value).length > 0);
function vTilePy(word) {
  const e = vTiles.value[word];
  if (!e || !e.py) return "";
  return settings.toneColors ? colorPinyin(e.py) : e.py;
}
function vTileEn(word) {
  return vTiles.value[word]?.en || "";
}
